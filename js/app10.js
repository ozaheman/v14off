...
async function handleDesignViewSwitch(event) {
    ...
    const addTaskBtn = App.DOMElements['add-task-btn'];
    ...
    addTaskBtn.style.display = showFiltersAndButton ? 'inline-block' : 'none';
    ...
    if (view === 'board') {
        ...
        if (jobNo) {
            ...
            if (scrumData) {
             ...
             App.DOMElements['scrum-board-container'].innerHTML = '<p style="text-align:center; padding-top: 50px; color: #888;">Please select a project to view its design board.</p>';
        }
    } else {
        ...
        const allProjects = await DB.getAllProjects();
        if (view === 'calendar') {
            App.DOMElements['design-calendar-container'].style.display = 'block';
...
function applyScrumBoardFiltersxx() {
    const selectedDept = App.DOMElements['department-filter'].value;
    const selectedStaffId = App.DOMElements['staff-filter'].value;
    document.querySelectorAll('#scrum-board-container .scrum-card').forEach(card => {
        const deptMatch = !selectedDept || card.dataset.department === selectedDept;
        const staffMatch = !selectedStaffId || card.dataset.assigneeId === selectedStaffId;
        card.classList.toggle('filtered-out', !(deptMatch && staffMatch));
    });
}
...
function setupEventListeners() {
    ...
    App.DOMElements['add-task-btn']?.addEventListener('click', showAddTaskModal);
...
function cacheDOMElements() {
    const ids = [
        ...
        'add-task-btn', 'add-task-modal', 'add-task-modal-close-btn', 'add-task-modal-title', 'add-task-name', 'add-task-planned-duration', 'add-task-department', 'add-task-assignee', 'add-task-duedate', 'add-task-to-similar', 'save-new-task-btn',
