    ...
    async function handleDesignViewSwitch(event) {
        ...
        if (view === 'board') {
            ...
            if (jobNo) {
                ...
                if (scrumData) ScrumBoard.render(scrumData, staffList, handleTaskStatusUpdate, showTaskModal, DEPARTMENT_COLORS);
            } else {
 ...
 async function updateDashboardSummary(projects) {
    ...
    async function updateActivePreview(tabId) {
        ...
        if (typeof PROJECT_DOCUMENT_TEMPLATES === 'undefined' || typeof PROJECT_LETTER_TEMPLATES === 'undefined') {
            console.error("Template objects not found. Make sure project_document_formats.js and letter_formats.js are loaded correctly and do not have syntax errors (like duplicate const declarations).");
            return;
        }
        if (!currentProjectJobNo) return;
        ...
        const renderMap = {
            ...
            'project-letter': () => renderProjectLetterPreview()
        };
...
async function renderMasterFileGallery(containerEl, jobNo, category) {
    ...
    async function handleNewProject() {
        ...
        let newProject = { 
            jobNo, agreementDate: new Date().toISOString().split('T')[0], scope: {}, notes: {}, invoices: [], 
   ...
   function getFeeDistribution(projectData) {
    ...
    function cacheDOMElements() {
        const ids = [
            ...
            'design-studio-btn', 'back-to-dashboard-btn-from-design', 'design-project-selector', 'scrum-board-container', 'design-calendar-container', 'design-view', 'design-agenda-container', 'design-assignee-container',
            ...
            'payment-cert-no', 'generate-new-cert-btn', 'cert-history-body', 'resourcePredictionOutput', 'qr-code',
    ...
    // The populateControlTabs, populateStaticControls, getFormDataFromUI, and other utility functions remain unchanged
    // from the previous complete version and are omitted here for brevity but are part of the full file.
    // ...
    function populateControlTabs() {
        ...
        DOMElements['vendors-tab'].innerHTML = `<h3>Vendor Management</h3>${createDocCategoryHTML(vendorDocCats)}`;
